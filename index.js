...
const { scheduleMidnightTask, writeLog } = require('./lib/log');
    ...
    sock.ev.on('messages.upsert', async (m) => { // CHAT MASUK
        try {  // console.log(JSON.stringify(m, null, 2))
           ...
           if (
            ...
            /* --------------------- Send Message ---------------------- */
            try {
                if(config.status.toLocaleLowerCase() == 'on'){
        ...
        if (connection === 'close') {
            ...
            switch (reason) {
                ...
                default:
                    ...
                    return await connectToWhatsApp(dataConnection);
                    break;
            }

        } else if (connection === 'open') {

            await sock.sendMessage('6285246154386@s.whatsapp.net', { text: "Bot Connected" });
